...
----- BUILD -----
    ...
    on createARDUser_(sender)
        ...
        if ardEnabled is equal to true then
            try
                ...
                set variableVariable to netBootDmgMountPath & "/Library/Application Support/AutoImagrNBI/Settings/ARDUser.plist"
                -- Encode ardUsername
    ...
    on setTimeServerAndZone_(sender)
            try
                ...
                set my buildProcessProgressBar to buildProcessProgressBar + 1
                -- Imagr Plist location on mounted volume
                set variableVariable to netBootDmgMountPath & "/Library/Application Support/AutoImagrNBI/Settings/TimeSettings.plist"
                ...
                installAutoImagrNBIStartup_(me)
            on error
                --Log Action
    ...
    -- Install AutoImagrNBIStartup.pkg
    on installAutoImagrNBIStartup_(sender)
        try
            -- Imagr Plist location on mounted volume
            set variableVariable to netBootDmgMountPath & "/Library/Application Support/AutoImagrNBI"
            -- Update Build Process Window's Text Field
            set my buildProcessTextField to "Installing AutoImagrNBI LaunchDaemon & required files"
            delay 0.1
            --Log Action
            set logMe to "Trying to install AutoImagrNBIStartup files"
            logToFile_(me)
            -- Update build Process ProgressBar
            set my buildProcessProgressBar to buildProcessProgressBar + 1
            --Log Action
            ...
            set my buildProcessProgressBar to buildProcessProgressBar + 1
            --Log Action
            set logMe to "Trying to set ownership to root:wheel on " & quoted form of netBootDmgMountPath & "/Library/LaunchDaemons/com.AutoImagrNBI.boot.plist"
            logToFile_(me)
            -- Correct ownership
            do shell script "/usr/sbin/chown root:wheel " & quoted form of netBootDmgMountPath & "/Library/LaunchDaemons/com.AutoImagrNBI.boot.plist" user name adminUserName password adminUsersPassword with administrator privileges
            --Log Action
            set logMe to "Set ownership to root:wheel on " & quoted form of netBootDmgMountPath & "/Library/LaunchDaemons/com.AutoImagrNBI.boot.plist"
            logToFile_(me)
            --Log Action
            set logMe to "AutoImagrNBIStartup files installed successfully"
            logToFile_(me)
            ...
            set my userNotifyErrorHidden to false
            -- Set Error message
            set my userNotifyError to "Error: Installing AutoImagrNBI LaunchDaemon & required files"
            -- Notify of errors or success
            userNotify_(me)
        end try
    end installAutoImagrNBIStartup_
    ...
    -- Copy Imagr.app selected earlier
    on copyImagrApp_(sender)
        try
            -- Update Build Process Window's Text Field
            set my buildProcessTextField to "Copying Imagr.app"
            delay 0.1
            -- Update build Process ProgressBar
            set my buildProcessProgressBar to buildProcessProgressBar + 1
            -- Cut the trailing /
            ...
            --Log Action
            set logMe to "Imagr.app to copy resides " & selectedAppPathToCopy
            logToFile_(me)
            -- Copy location for Imagr.app
            set variableVariable to netBootDmgMountPath & "/Applications/"
            -- Copy path of Imagr.app
            set copiedAppPath to netBootDmgMountPath & "/Applications/Imagr.app"
            --Log Action
            set logMe to "Imagr.app to be copied to " & copiedAppPath
            logToFile_(me)
            -- Copy Imagr.app & log
    ...
    on writeImagrPlist_(sender)
        try
            ...
            installImagrLaunchAgent_(me)
        on error
            --Log Action
            set logMe to "Error: Writing the Imagr plist"
            logToFile_(me)
            -- Set to false to display
            set my userNotifyErrorHidden to false
            -- Set Error message
            set my userNotifyError to "Error: Writing the Imagr plist"
            -- Notify of errors or success
            userNotify_(me)
        end try
    end writeImagrPlist_

    -- Install ImagrLaunchAgent
    on installImagrLaunchAgent_(sender)
        try
            -- Update Build Process Window's Text Field
            set my buildProcessTextField to "Installing Imagr LaunchAgent"
            delay 0.1
            -- Update build Process ProgressBar
            set my buildProcessProgressBar to buildProcessProgressBar + 1
            --Log Action
            ...
            set my buildProcessProgressBar to buildProcessProgressBar + 1
            --Log Action
            set logMe to "Correcting ownership on " & quoted form of netBootDmgMountPath & "/Library/LaunchAgents/com.AutoImagrNBI.Imagr.plist"
            logToFile_(me)
            -- Correct ownership
            do shell script "/usr/sbin/chown root:wheel " & quoted form of netBootDmgMountPath & "/Library/LaunchAgents/com.AutoImagrNBI.Imagr.plist" user name adminUserName password adminUsersPassword with administrator privileges
            --Log Action
            set logMe to "Set ownership to root:wheel on " & quoted form of netBootDmgMountPath & "/Library/LaunchAgents/com.AutoImagrNBI.Imagr.plist"
            logToFile_(me)
            -- Update build Process ProgressBar
            set my buildProcessProgressBar to buildProcessProgressBar + 1
            --Log Action
            set logMe to "Trying to correct permissions on " & quoted form of netBootDmgMountPath & "/Library/LaunchAgents/com.AutoImagrNBI.Imagr.plist"
            logToFile_(me)
            -- Making  writable
            do shell script "/bin/chmod 644 " & quoted form of netBootDmgMountPath & "/Library/LaunchAgents/com.AutoImagrNBI.Imagr.plist" user name adminUserName password adminUsersPassword with administrator privileges
            --Log Action
            set logMe to "Set permissons on " & quoted form of netBootDmgMountPath & "/Library/LaunchAgents/com.AutoImagrNBI.Imagr.plist to 644"
            logToFile_(me)
            -- Add any additional certs if specified
            importAdditionalCerts_(me)
        on error
            --Log Action
            set logMe to "Error: Installing Imagr LaunchAgent"
            logToFile_(me)
            -- Set to false to display
            set my userNotifyErrorHidden to false
            -- Set Error message
            set my userNotifyError to "Error: Installing Imagr LaunchAgent"
            -- Notify of errors or success
            userNotify_(me)
        end try
